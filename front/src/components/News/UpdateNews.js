...
const UpdateNews = () => {
  ...
  const removeImage = async (imageId, isMain = false) => {
    ...
    try {
      ...
      await axios.delete(`http://localhost:8082/api/news/${id}/images/${imageId}`, {
        headers: {
          "Authorization": `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:8082/api/news/${id}`, formDataToSend, {
        headers: {
            ...
            <div className="form-group">
              ...
              {(previewAvatar || existingImages.mainImage) && (
                <div className="image-preview mt-1" style={{ position: 'relative' }}>
                  <img 
                    src={previewAvatar || `http://localhost:8082/api/files/${existingImages.mainImage.id}`}
