...
const News = () => {
  ...
  const fetchNews = async (token) => {
    try {
      const response = await axios.get(`http://localhost:8082/api/news/${id}`, {
        headers: {
  ...
  const deleteNews = async () => {
    if (window.confirm("Вы действительно хотите удалить эту новость?")) {
      try {
        ...
        await axios.delete(`http://localhost:8082/api/news/${id}`, {
          headers: {
            "Authorization": `Bearer ${token}`
          }
        });
    ...
    <div className="container">
      <div className="news-detail">
        <div className="news-header">
          ...
          {news.avatar && (
            <img
              src={`http://localhost:8082/api/files/${news.avatar.id}`}
              alt={news.title}
        ...
        <div className="news-info">
          ...
          {news.author?.avatar && (
            <div className="author-avatar">
              <img
                src={`http://localhost:8082/api/files/${news.author.avatar.id}`}
                alt="Аватар автора"
          ...
          <div className="news-gallery">
            ...
            <div className="images-grid">
              {news.images.map((image, index) => (
                <img
                  key={index}
                  src={`http://localhost:8082/api/files/${image.id}`}
      ...
      {selectedImage && (
        <div className="modal-overlay" onClick={closeImageModal}>
          <div className="modal-content">
            <img
              src={`http://localhost:8082/api/files/${selectedImage.id}`}
